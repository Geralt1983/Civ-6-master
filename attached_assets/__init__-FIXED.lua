-- Civ6Companion mod initialization
print("DEBUG: Civ6Companion mod loading...")

function OnTurnBegin()
    print("DEBUG: OnTurnBegin triggered!")
    
    local iPlayer = Game.GetLocalPlayer()
    if not iPlayer or iPlayer == -1 then 
        print("DEBUG: Invalid player")
        return 
    end
    
    local pPlayer = Players[iPlayer]
    if not pPlayer then 
        print("DEBUG: No pPlayer")
        return 
    end
    
    local pPlayerConfig = PlayerConfigurations[iPlayer]
    if not pPlayerConfig then 
        print("DEBUG: No pPlayerConfig")
        return 
    end
...
if Events and Events.TurnBegin then
    print("DEBUG: Registering TurnBegin event...")
    Events.TurnBegin.Add(OnTurnBegin)
else
    print("DEBUG: Events.TurnBegin not available!")
end

print("DEBUG: Civ6Companion mod loaded successfully!")
